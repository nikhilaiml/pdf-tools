...
import { Loader2, Type, Calendar, Pen, Download, X, HelpCircle, Grip, Trash2, Check, Undo, Cloud, CheckCircle } from 'lucide-react';
...
const DraggableItem = ({ element, updatePos, updateContent, remove, isSelected, onSelect }: {
    element: DraggableElement,
    updatePos: (id: string, dx: number, dy: number) => void,
    ...
    return (
        <div
            ...
            onPointerDown={(e) => {
                onSelect();
                // We need to prevent immediate drag start if interacting with inputs in some cases,
...
const SignatureModal = ({ onClose, onSave }: { onClose: () => void, onSave: (dataUrl: string) => void }) => {
    ...
    const handleSave = () => {
        ...
        // Upload logic handled directly in input
    };
...
const FillAndSignPdfClient = () => {
    ...
    // DND Sensors for better touch capability
    ...
    useEffect(() => {
        ...
        const renderPage = async () => {
            ...
            // Adjust scale based on container width if needed for mobile responsiveness
            const containerWidth = containerRef.current?.getBoundingClientRect().width || 600;
            const unscaledViewport = page.getViewport({ scale: 1 });
            const fitScale = Math.min(containerWidth / unscaledViewport.width, 1.5); // Max 1.5x zoom

            // Note: we can map exact user intended scale later, for now we fit-width mostly
            const finalScale = fitScale;
            // setScale(finalScale); // Update state so coordinate mapping is correct! - loop risk?
            ...
            // We need to store this render scale to map coordinates later
            setScale(finalScale);
        };
        renderPage();
    }, [pdfProxy, pageNum]); // Dependency on scale removed to avoid loop if we set it inside
    ...
    const addSignature = (dataUrl: string) => {
        ...
        setElements(prev => [...prev, {
            ...
            y: 100,
            width: 150,
            height: 75 // Aspect ratio might be issue, will adjust later or let user resize
        }]);
    };
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            for (const el of elements) {
                // Coordinate Mapping
                // Canvas (0,0) is Top-Left. PDF (0,0) is typically Bottom-Left.
                // scale is pixels per point? No, viewport scale.
                // PDF Point = Canvas Pixel / scale.

                const pdfX = el.x / scale;
                // PDF Y = PageHeight - (CanvasY / scale) - (ElementHeightInPoints)
                // For text, baseline is roughly bottom, so minus font size is approximate

                if (el.type === 'text' || el.type === 'date') {
                    const fontSize = (el.fontSize || 16); // * (1/scale)? No, font size is usually 'pt' logic or direct px? 
                    // pdf-lib size is in points. 
                    // if we render input at 16px CSS, that's roughly 16 points if 1px=1pt at 72DPI (standard pdf-lib assumption usually)
                    // We need to test this alignment.

                    const pdfY = pageHeight - (el.y / scale) - (fontSize * 0.8); // 0.8 is heuristic for baseline
                    ...
                    const pngImage = await pdfDoc.embedPng(el.content); // Assuming signature is PNG (data url from canvas)
            ...
            {!file ? (
                ...
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="absolute inset-0 z-0">
                                    {elements.map(el => (
                                        <DraggableItem
                                            key={el.id}
                                            element={el}
                                            updatePos={() => { }} // Controlled by DndContext
                    ...
                    <p className="text-center text-gray-500 text-sm flex items-center justify-center gap-1.5 mt-2">
                        ...
                        Your PDF is processed securely and never stored on our servers.
                    </p>
                </div>
            )}
        </div>
...
export default FillAndSignPdfClient;
